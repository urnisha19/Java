...
// This program reads a file sequentially and displays the contents based on the type of account the user requests(credit balance, debit balance or zero balance).
...
public class CreditInquiry
{
    ...
    {
        //get user's request (e.g., zero, credit or debit balance). method getRequest display the menu options
        MenuOption accountType = getRequest();
        
        while (accountType != MenuOption.END) //loop until the user specifies that the program should terminate
        {
            switch (accountType)
            /*display a header for the current set of records to be output to the screen*/
            ...
            readRecords(accountType);//calls method readRecords, which loops through the file and reads every record
    ...
    private static void readRecords(MenuOption accountType)
    {
        ...
        /*try-with-resources statement to create a Scanner that opens the file for reading.
        try-with-resources will close its resource(s) when the try block terminates successfully or due to an exception. 
        The file will be opened for reading with a new Scanner object each time readRecords is called, so that we can again read 
        from the beginning of the file*/
            
        {
            while (input.hasNext()) // more data to read:  read a record
            {
                ...
                /*method shouldDisplay determine whether the current record satisfies the account type requested*/                   
